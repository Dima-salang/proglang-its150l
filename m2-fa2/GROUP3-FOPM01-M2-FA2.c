...
void draw_gantt_chart(struct process processes[], int arr_len, int sum_burst_time);
...
int main() {
    ...
    draw_gantt_chart(processes, arr_len, sum_burst_time(processes, arr_len));
...
void draw_gantt_chart(struct process processes[], int arr_len, int sum_burst_time) {
    ...
    while (current_time <= sum_burst_time+buffer_time) {
        // check if there is a process that arrives
    ...
    // TO-DO: print the gantt chart for sjf
}
